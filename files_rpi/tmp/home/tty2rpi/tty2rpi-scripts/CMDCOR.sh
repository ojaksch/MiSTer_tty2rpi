...
if [ "${PLAYVIDEO}" = "yes" ]; then
  if [ "${FBUFDEV}" = "yes" ]; then
    ...
    [ -f "${MEDIA}" ] && ffmpeg -loglevel quiet -re -i "${MEDIA}" -an -c:v rawvideo -pix_fmt rgb565le -f fbdev -vf "scale=w=${WIDTH}:h=${HEIGHT}:force_original_aspect_ratio=decrease,pad=${WIDTH}:${HEIGHT}:(ow-iw)/2:(oh-ih)/2" ${FBDEVICE}
  else
    if [ -f "${MEDIA}" ]; then
      case "${VIDEOPLAYER}" in
